...
**Not run successfully** — tooling absent on this box:

| Tool | Status |
|------|--------|
| JDK / `java` | Not installed (`java: command not found`) |
| `ANDROID_HOME` | Unset; no Android SDK detected |
| Gradle Wrapper JAR | Properties present; `gradle-wrapper.jar` not bundled (cannot bootstrap without network+JDK) |

Complete Kotlin / Gradle / Compose **sources** for MOB-1.1 → 1.4 (+ light 1.5 stubs) are in place.  
Once JDK 17+ and Android SDK (compileSdk 35) are available:

```bash
cd /workspace/confluence_app
# Generate wrapper jar if needed, then:
...
- No git push / no GitHub contact from this scaffold task (repo may already be a local clone)
- No exchange SDKs / no trade execution
- No Compose Canvas candle engine
