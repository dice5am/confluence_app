...
Public facade (no-params overloads keep day-one defaults):

- `IndicatorCalc.evaluate(bars, cutoff)` → `DayOneIndicators` (as-of the last fenced bar)
- `IndicatorCalc.evaluate(bars, cutoff, params)`
- `IndicatorCalc.evaluateAsOf(bars, cutoff, asOfCloseTimeMs)` — global fence, then `closeTimeMs <= asOfCloseTimeMs`
- `IndicatorCalc.evaluateAsOf(bars, cutoff, asOfCloseTimeMs, params)`
- `IndicatorCalc.onClosedBar(current, bar)` — recomputes with `current.params`
- `IndicatorCalc.onClosedBar(current, bar, params)`

`IndicatorParams` (defaults = today's five autos):

| Field | Default |
|-------|---------|
| `movingAverages: List<MaSpec>` | `EMA:9`, `EMA:21`, `SMA:50`, `SMA:200` |
| `rsiPeriod` | 14 |
| `ichimoku` (`tenkan` / `kijun` / `senkouB` / `displacement`) | 9 / 26 / 52 / 26 |
| `volumeSmaPeriod` | 20 |
| `volumeProfileLookback` | 24 (fixed last-N closed bars ending at as-of) |

Omitting params or passing `IndicatorParams.DEFAULT` is equivalent to the no-params overloads (same series). Arbitrary positive MA periods are allowed (at least 8 / 21 / 50 / 200). Multi-MA results are a keyed list (`DayOneIndicators.movingAverages`, keys `EMA:9` / `SMA:50`, lookup via `ma(type, period)` / `ma(key)`). Legacy fields `ema9` / `ema21` / `sma50` / `sma200` / `rsi14` / `volumeSma20` remain as shims for the day-one specs (empty/undefined when that spec is omitted).
...
Phase A look-ahead proof: [`ALT-PHASE-A-calc-honesty.md`](ALT-PHASE-A-calc-honesty.md). Params do **not** change as-of fence semantics.
...
Candle-only rule: **last N closed bars of the provided series**. Default **N = 24** (`IndicatorParams.volumeProfileLookback`). No manual range UI.

- Prefer callers pass **1h+** (ALT-1.2). The engine does not roll 1m up.
- On 1h with N=24 this is ~24 hours of 1h candles. It is **not** an America/Toronto cash session and **not** tick/aggTrade VP.
...
If fewer than N bars are available, all fenced bars are used and `lookbackLimited = true`.
...
| Volume profile | Fixed last-N lookback of the *provided* window (default 24); closed-bar distribution, not ticks. Not a Toronto session. Full-series VP is as-of last bar — use `computeAsOf` for earlier bars. |
