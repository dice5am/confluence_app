...
Public facade: `IndicatorCalc.evaluate(bars, cutoff)` → `DayOneIndicators`.  
...
| Volume profile | Fixed 24-bar lookback; closed-bar distribution, not ticks. Not a Toronto session. |
...
`:domain-indicators` unit tests load packaged `data/src/main/assets/md_snapshot/` (and the services fixtures mirror), apply the fence, and assert finite outputs on fenced **1h** plus **1d** / **1w** honesty (SMA-200 warmup on 1w). An explicit test drops `closeTimeMs > cutoffMs` bars.
